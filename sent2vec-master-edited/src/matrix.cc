...
void Matrix::exportData() const {
  std::ofstream out("/mnt/c/users/gunu1/documents/ucsf_rad/acr_project/resources/test_v2.csv");
  
  // float out_arr [700];
  for (int row = 0; row< m_; ++row) {
  // for (int row = 0; row< 100; ++row) {
    // int file_num = floor(row/20000);
    // char buffer[100];
    // sprintf(buffer,"/mnt/c/users/gunu1/documents/ucsf_rad/biosentvec-data/embs/part_%03d/%07d.json", file_num, row);
