...
/* OK SO the point of this function is to map a word to an index in the dictionary.
*/
...
std::vector<int32_t> Dictionary::get_word2int() const {
  return word2int_;
}
...
void Dictionary::addNgrams(std::vector<int32_t>& line, int32_t n) const {
  ...
  for (int32_t i = 0; i < line_size; i++) {
    ...
    for (int32_t j = i + 1; j < line_size && j < i + n; j++) {
      ...
      h = h * 116049371 + line[j];
      std::cout << nwords_ << ' ' << args_->bucket << ' ' << nwords_ + (h % args_->bucket) << std::endl;
      line.push_back(nwords_ + (h % args_->bucket));
    }
  }
}
